...
const changesForMediumDevices = () => {
	if (window.matchMedia('(max-width: 1023px)').matches) {
		/* можем не удалять событие для средних и маленьких разрешений, так как изначально мы получаем из DOM дерева только первый
		progress bar под большие разрешения, а значит updateTimeCurrentSong использует только его*/
