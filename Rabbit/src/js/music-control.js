...
const animationTxt = () => {
	...
	if (txtWrapper.firstElementChild.innerHTML.length > 14) { //text > 15 symbols; 
		txtWrapper.firstElementChild.style.animation = "5s cubic-bezier(0, 0.3, 1, 0.7) 1s infinite normal none running slidein";
	} else {
		txtWrapper.firstElementChild.style.animation = "";
	}
	if (txtWrapper.lastElementChild.innerHTML.length > 14) { //text > 15 symbols
		txtWrapper.lastElementChild.style.animation = "";
		txtWrapper.lastElementChild.style.animation = "5s cubic-bezier(0, 0.3, 1, 0.7) 1s infinite normal none running slidein";
	} else {
		txtWrapper.lastElementChild.style.animation = "";
	}
};
...
const changesForMediumDevices = () => {
	if (window.matchMedia('(max-width: 1023px)').matches) {
		...
		element.classList.add("music-player__song-progress-area");
		element.classList.add("music-player__song-progress-area-for-small-devices");
		element.innerHTML = `
			<div class="music-player__song-progress">
				<audio id="main-audio" src="music/${allMusic[musicIndex - 1].src}.mp3"></audio>
			</div> 
		`;
		musicPalyer.prepend(element); */	
	}
};
